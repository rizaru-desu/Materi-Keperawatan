...
export default class MateriKep extends Component {
  constructor(props) {
    super(props);
    this.state = {
      //Profile
  ...
  _getMateri = () => {
    ...
    axios
      .post(`https://materi-keperawatan-apps.herokuapp.com/API-MKEP`, {
        nameJSON: "materi.json",
      })
      .then((res) => {
        this.setState({ MATERI: res.data, _loader: false });
      });
  };

  render() {
    const Item = ({ title, images, index }) => (
      <TouchableScale
        onPress={() => {
          this.props.navigation.navigate("ReaderHtml", { urlMateri: index });
        }}
      >
        <View
          style={{
    ...
    const renderItem = ({ item }) => (
      <Item title={item.title} images={item.images} index={item.index} />
    );
